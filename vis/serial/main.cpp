...
{
    // return 0 on normal execution, -1 on failed convergence
    ...
    for (it=0; it<max_newton_iters; it++)
    {
        ...
        // update solution
        ss_axpy(x_new, -1.0, deltax, options.N);
    }
    iters_newton += it+1;
...
int main(int argc, char* argv[])
{
...
#ifdef _VISIT_
  runMode = false;
  //VisItOpenTraceFile("/tmp/trace.txt");
  char * env = VisItGetEnvironment();
  ...
  VisItInitializeSocketAndDumpSimFile(
    ...
    coords_x.init(nx,1);
    for (int i = 0; i < nx; i++)
      coords_x[i] = options.dx*i;
    coords_y.init(ny,1);
    for (int i = 0; i < ny; i++)
      coords_y[i] = options.dx*i;
#endif
    ...
    ////////////////////////////////////////////////////////////////////
    // write final solution to XDMF file for visualization. Readable by Paraview and Visit
    ////////////////////////////////////////////////////////////////////
