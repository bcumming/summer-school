...
subroutine diffusion(u, s)
    ...
    ! BEGIN PARALLEL OPENMP REGION
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !$omp parallel  private(i,j) firstprivate(alpha)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    !---------------------------------------------
    ! interior points with parallel for loop
    !---------------------------------------------
    ! the interior grid points private(i,j) firstprivate(alpha)
    !$omp do
    do j = 2, jend
        do i = 2, iend
    ...
    !$omp critical
    !---------------------------------------------
    ! each boundary is computed with an OpenMP task
    !---------------------------------------------
    ! the east boundary
    !---------------------------------------------
    i = options%nx
