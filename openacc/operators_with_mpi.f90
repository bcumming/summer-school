...
subroutine diffusion(u, s)
    ...
    !!$acc data create(buffN, buffS, buffE, buffW) copy(s, u, x_old, bndN, bndE, bndW, bndS)
    ...
    if (domain%neighbour_north>=0) then
        ...
        ! post send
        !!$acc host_data use_device(buffN)
